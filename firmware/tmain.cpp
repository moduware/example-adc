...
void np_api_loop() {
	...
	*/
	...
	unsigned char data_to_send[2]; //Define 2 element length array (8 Bit each element)
	unsigned int potentiometer_value;

	potentiometer_value = analogRead(POT);	 //Read input

	//Divide ADC (10 Bit input) value in 2 element array
	data_to_send[0] = (potentiometer_value >> 8) & 0xff;
	data_to_send[1] = potentiometer_value & 0xff;

	np_api_upload(0x2800, data_to_send, 2); //send potentiometer's value to app from address 2800

	// delay before sending next value
	delay(500);
}
